...
//
// This is a modified version of the original derivative_test.cpp which makes
// use of a second extrapolation routine to increase the accuracy of the differentiation
// this new routine makes use of the already existing extrap_diff function, calling it twice.
// the original extrapolation's error scaled like h^4, but our new function's errors
// scales like h^6. This can be seen visually from the plot created as well as
// from the fit concerning the relevant region. Our fit yields a slope of 6 When
// plotting on a log scale indicating that the error scales like h^6.
//
...
//
// Notes:
// * compile with: "make -f make_derivative_test"
  ...
  while (h >= hmin)
    {
      ...
      diff_extrap = extrap_diff2 (x, h, &funct, params_ptr);
