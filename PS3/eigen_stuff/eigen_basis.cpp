...
//
// This is a modified version of the original eigen_basis.cpp to calculate
// the wave function as a function of r and output it to a dat file
//
// Programmer: Matt Bundas bundasma@msu.edu
//
// Revision history:
//    03-27-2019 started and finished
//
// Notes:
// * compile with: "make -f make eigen_basis.cpp"

//It seems like the way to determine the best dimension and best value for b is
//to see at which values the calculated wave function is closest to the actual
//wave function. My wave functions do not resemble any sort of convergence so I
//expect something might not be going correctly. I am confused as to what it could be
//as I believe I am summing the eigenvector 1 and multiplying it by the basis function
...
main ()
{
  ...
  int dimension = 5;
  //open file stream
  ofstream out ("eigen_basis5_b_2_0.dat");
  ...
  //only go to i=0 so we leave off on first eigenvalue
  for (int i = 0; i < 1; i++)
    {
    ...
    for (double r = rmin; r<rmax; r+=delta_r){
      for (int ex=0; ex < dimension; ex++){
        //get the wave function by summing the eigenvectors and radial functions
        u_r_1 += gsl_vector_get (Eigval_ptr, ex)*ho_radial (1, 0, b_ho, r);}

    out << scientific << r << "    " << u_r_1 * pow(1./factor1,.25) << endl;
    //reset the wave function value
    u_r_1 = 0;}
